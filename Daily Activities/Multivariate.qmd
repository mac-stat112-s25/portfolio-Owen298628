...
```{r}
satdata <- read_excel("/Users/owenlindstrom/Documents/2025/Stat 112/portfolio-Owen298628/Daily Activities/SATPart.xlsx")
satdata <- satdata %>%
  mutate(State = tolower(State),  
         State = gsub("\u00A0", " ", State),  # Convert non-breaking spaces to normal spaces
         State = gsub(" +", " ", State),  # Remove any extra spaces
         State = trimws(State))  # Trim leading/trailing spaces
map_sat_2 <- states_map %>%
  left_join(satdata, by = c("region" = "State"))
```


```{r State Map}
states_map <- map_data("state")  # Get US states map data
education <- education %>%
  mutate(State = tolower(State))  # Convert state names to lowercase
education <- education %>%
  mutate(State = tolower(State),   # Convert to lowercase
         State = gsub(",", " ", State),  # Replace commas with spaces
         State = trimws(State))    # Remove extra spaces
map_sat <- states_map %>%
  left_join(education, by = c("region" = "State"))
```

```{r}
ggplot(data = map_sat, aes(x = long, y = lat, group = group, fill = sat)) +
  geom_polygon(color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "SAT Avg") +
  theme_void() +
  theme(legend.position = "bottom", legend.key.width = unit(2, "cm"), legend.key.height = unit(0.5, "cm"), plot.title = element_text(hjust = 0.5, size = 14, face = "bold")) +
  ggtitle("State Average SAT Scores")
```

```{r}
ggplot(data = map_sat_2, aes(x = long, y = lat, group = group, fill = SAT)) +
  geom_polygon(color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "SAT Participation Rate") +
  theme_void() +  
  theme(
    legend.position = "bottom", legend.key.width = unit(2, "cm"), legend.key.height = unit(0.5, "cm"),
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold")  # Center title
  ) +
  ggtitle("SAT Participation Rate")

```
```{r}
ggplot(data = map_sat, aes(x = long, y = lat, group = group, fill = expend)) +
  geom_polygon(color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "Expenditure") +
  theme_void() +
  theme(legend.position = "bottom", legend.key.width = unit(2, "cm"), legend.key.height = unit(0.5, "cm"), plot.title = element_text(hjust = 0.5, size = 14, face = "bold")) +
  ggtitle("State Education Expenditure")
```

```{r}
# Print unique state names from each dataset
print("States from map_data:")
print(unique(states_map$region))  # Should be lowercase state names with spaces

print("States from education:")
print(unique(education$State))  # Check if they match states_map$region

print("States from satdata:")
print(unique(satdata$State))  # Check if they match states_map$region
...
The midwest has the highest average SAT scores2
...
MN is high on the SAT performance related metrics and low on everything else. MN is similar t