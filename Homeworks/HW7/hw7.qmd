...
```{r}
running_112 <- read.csv("/Users/owenlindstrom/Documents/2025/Stat 112/portfolio-Owen298628/Homeworks/HW7/running_112.csv")
```
...
The data shows that there are some missing values in the `net` variable, which we will likely need to clean. The distribution of `nruns` and `previous` variables indicates most runners have participated fewer times, but some have participated multiple times.
:::
...
running_112 <- running_112 %>%
  separate(name.yob, into = c("name", "yob"), sep = " (?=[0-9]{4})") %>%
...
```{r}
ggplot(running_112, aes(x = age)) +
  geom_histogram(binwidth = 1, fill = "skyblue", color = "white") +
  labs(title = "Age Distribution of Runners", x = "Age", y = "Count") +
  theme_minimal()
```
...
```{r}
ggplot(running_112, aes(x = net)) +
  geom_histogram(binwidth = 5, fill = "lightgreen", color = "white") +
  labs(title = "Running Time Distribution", x = "Time (minutes)", y = "Count") +
  theme_minimal()
```
...
running_112 %>% filter(net > 1000)
```

::: {.callout-note title="Explanation"}

The odd data point (`net > 1000`) is likely a data entry error, intended as approximately 100 minutes. Removing it is justified because it's clearly not realistic and skews analyses significantly.
...
running_112 <- running_112 %>% filter(net < 1000 | is.na(net))
...
```{r}
age_summary <- running_112 %>%
  group_by(age) %>%
  summarize(fastest = min(net, na.rm = TRUE),
            average = mean(net, na.rm = TRUE),
            slowest = max(net, na.rm = TRUE))

ggplot(age_summary, aes(x = age)) +
  geom_line(aes(y = fastest, color = "Fastest")) +
  geom_line(aes(y = average, color = "Average")) +
  geom_line(aes(y = slowest, color = "Slowest")) +
  labs(x = "Age", y = "Running Time (minutes)", title = "Running Times by Age", color = "Legend") +
  theme_minimal()
```
...
- Young runners generally have faster average times.
- Running times increase steadily as age increases.
- Older runners show more variability in their running times.
:::
...
```{r}
# Numerical evidence
running_112 %>% count(age) %>% arrange(desc(age)) %>% head(10)
```

::: {.callout-note title="Explanation"}

Volatility in older ages is due to fewer runners at older ages. With fewer observations, extreme values significantly affect summary statistics.
:::
...
I wonder how individual runner experience influences their running time as they age.
...
```{r}
ggplot(running_112, aes(x = age, y = net)) +
  geom_point(alpha = 0.3) +
  labs(title = "Running Time vs Age", x = "Age", y = "Running Time") +
  theme_minimal()
```
...
```{r}
ggplot(running_112, aes(x = age, y = net, color = as.factor(previous))) +
  geom_point(alpha = 0.3) +
  labs(title = "Running Time vs Age by Prior Experience", x = "Age", y = "Running Time", color = "Prior Runs") +
  theme_minimal()
```

::: {.callout-note title="Caveat"}

Previous runs are a proxy because someone could have extensive running experience outside this dataset, which isn't captured by `previous`.
:::
...
Prior running experience significantly affects the relationship between age and running time, with experienced runners generally showing less increase in time with age.
...
running_112 %>% summarize(unique_runners = n_distinct(name), min_runs = min(nruns), max_runs = max(nruns))
...
running_112 %>%
  filter(nruns >= 7) %>%
  ggplot(aes(x = age, y = net, group = name)) +
  geom_smooth(method = "lm", se = FALSE, alpha = 0.2) +
  labs(title = "Individual Trends (≥7 runs)", x = "Age", y = "Running Time") +
  theme_minimal()
```
...
`nruns` counts total participations, including those missing from our dataset due to NA values. Thus, observed data points per runner can be fewer than `nruns`.
...
```{r}
# Create new dataset
complete_runners <- running_112 %>%
  group_by(name) %>%
  filter(n() >= 7, !is.na(net)) %>%
  ungroup()

# Confirm that we have 511 data points
dim(complete_runners)
...
```{r}
ggplot(complete_runners, aes(x = age, y = net)) +
...
```{r}
 ggplot(complete_runners, aes(x = age, y = net, group = name)) +
...
Individual outcomes suggest running times increase with age. Individual runners show clear linear trends. Differences arise because group-level analysis masks individual-level trajectories.
